...
in~\cite{Areias-12b}. These benchmarks create \emph{worst case
  scenarios}, where we are able to show the lowest bounds of
performance that each design might achieve when applied/used in other
real world applications/programs.
...
propagated. This is the reason for overhead that we are observing in
...
difference between both strategies is significantly higher than in the
NS design.

In resume, the experimental results show that on average, the PAC
strategy does not seem to have a big impact in the performance,
however it still leaves room for further improvements, because the
difference between local and batched scheduling is higher in the FS
design than in the NS design.
